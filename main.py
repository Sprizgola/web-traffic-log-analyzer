from internal_lib.data_processing import *
...
if __name__ == "__main__":
    ...
    df = pd.read_csv(f"./parsed_data/parsed_dataframe.csv")
    ...
    for n_cluster in clusters_range:
        ...
        print(f"Adjusted Rand score: {adjusted_rand_score(y_train_true, y_train_pred)}")
        print("Predict data")
        ...
        pred_df = pd.DataFrame(
            ...
            columns=["session_id", "predicted_label"])
        ...
        y_test_pred = pred_df["predicted_label"].values

        plot_confusion_matrix(y_test_true, y_test_pred, f"plots/cm_{n_classes}_predict.png")

        print(f"Adjusted Rand score: {adjusted_rand_score(y_test_true, y_test_pred)}")
