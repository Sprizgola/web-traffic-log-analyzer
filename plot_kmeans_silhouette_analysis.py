...
numerical_features = ['session_duration', 'requests_count', 'mean_request', 'total_size',
                      ...
                      'add_to_cart', 'has_source', 'product_views', 'homepage_views']

index = df.index.to_numpy().reshape((df.shape[0], 1))
data = df[numerical_features].values
cat_col = df.has_source.values.reshape(df.shape[0], 1)
...
X = np.concatenate([index, X_scaled, cat_col], axis=1)
